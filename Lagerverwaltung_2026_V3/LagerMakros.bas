...
Sub Artikel_Suchen()
    ...
    For i = 5 To lastRow
        Dim zeile As String
        zeile = Join(Array( _
            CStr(wsA.Cells(i, 1).Value), CStr(wsA.Cells(i, 2).Value), _
            CStr(wsA.Cells(i, 3).Value), CStr(wsA.Cells(i, 4).Value), _
            CStr(wsA.Cells(i, 5).Value)), " ")
        If InStr(1, zeile, such, vbTextCompare) > 0 Then
