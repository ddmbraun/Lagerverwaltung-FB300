...
Sub AllesEinrichten()
    ...
    setupLog = "NeuerArtikel"
    NeuArtikelModul.Setup_NeuerArtikel
...
Sub Setup_ArtikelDetail()
    ...
    ' Spaltenbreiten (C und F gelbe Felder breiter fuer bessere Lesbarkeit)
    wsAD.Columns(1).ColumnWidth = 2
    wsAD.Columns(2).ColumnWidth = 16
    wsAD.Columns(3).ColumnWidth = 24
    wsAD.Columns(4).ColumnWidth = 14
    wsAD.Columns(5).ColumnWidth = 16
    wsAD.Columns(6).ColumnWidth = 24
    wsAD.Columns(7).ColumnWidth = 8

    wsAD.Visible = xlSheetHidden

    ' Events installieren (Buttons in Zeile 15 per Doppelklick bedienen)
    ArtikelDetail_Events_Installieren
...
' ================================================================
'  ArtikelDetail - Events installieren (kann auch separat ausgefuehrt werden)
'  Ohne Neuaufbau des Sheets ausfuehren falls Buttons bereits da sind.
' ================================================================
Sub ArtikelDetail_Events_Jetzt()
    ArtikelDetail_Events_Installieren
    MsgBox "Fertig! Buttons im ArtikelDetail-Sheet sind jetzt aktiv." & Chr(10) & _
           "Doppelklick auf SCHLIESSEN / BEWEGUNGEN / SPEICHERN.", _
           vbInformation, "Events installiert"
End Sub

Sub ArtikelDetail_Events_Installieren(Optional wsAD As Worksheet = Nothing)
    If wsAD Is Nothing Then Set wsAD = LagerMakros.GetSheet("ArtikelDetail")
    If wsAD Is Nothing Then
        MsgBox "ArtikelDetail-Sheet nicht gefunden!", vbExclamation
        Exit Sub
    End If
    ...
    c = c & "    If Target.Row = 15 Then" & Chr(10)
    c = c & "        If Target.Column >= 2 And Target.Column <= 3 Then NeueModule.ArtikelDetail_Schliessen" & Chr(10)
    c = c & "        If Target.Column >= 4 And Target.Column <= 5 Then NeueModule.ArtikelDetail_Bewegungen" & Chr(10)
    c = c & "        If Target.Column >= 6 And Target.Column <= 7 Then NeueModule.ArtikelDetail_Speichern" & Chr(10)
    c = c & "    End If" & Chr(10)
    c = c & "End Sub" & Chr(10)
    c = c & Chr(10)
...
OhneVBProject:
    MsgBox "Events konnten nicht installiert werden." & Chr(10) & Chr(10) & _
           "Bitte in Excel aktivieren:" & Chr(10) & _
           "Datei -> Optionen -> Trust Center -> Trust Center-Einstellungen" & Chr(10) & _
           "-> Makroeinstellungen -> 'Zugriff auf VBA-Projektobjektmodell vertrauen' -> AN", _
           vbExclamation, "VBA-Projektzugriff benoetigt"
End Sub
...
Sub Setup_BewPopup()
    ...
    ' Events installieren (SCHLIESSEN-Button per Doppelklick)
    BewPopup_Events_Installieren wsBP
...
' ================================================================
'  BewPopup - Events installieren (SCHLIESSEN per Doppelklick)
' ================================================================
Sub BewPopup_Events_Jetzt()
    BewPopup_Events_Installieren
    MsgBox "Fertig! SCHLIESSEN-Button in Bewegungshistorie ist jetzt aktiv." & Chr(10) & _
           "Doppelklick auf SCHLIESSEN (Zeile 2, rechts) schliesst das Popup.", _
           vbInformation, "Events installiert"
End Sub

Sub BewPopup_Events_Installieren(Optional wsBP As Worksheet = Nothing)
    If wsBP Is Nothing Then Set wsBP = LagerMakros.GetSheet("BewPopup")
    If wsBP Is Nothing Then
        MsgBox "BewPopup-Sheet nicht gefunden!", vbExclamation
        Exit Sub
    End If
    ...
    c = c & "Private Sub Worksheet_BeforeDoubleClick(ByVal Target As Range, Cancel As Boolean)" & Chr(10)
    c = c & "    Cancel = True" & Chr(10)
    c = c & "    If Target.Row = 2 And Target.Column = 6 Then NeueModule.BewPopup_Schliessen" & Chr(10)
    c = c & "End Sub" & Chr(10)
    c = c & Chr(10)
    c = c & "Private Sub Worksheet_Deactivate()" & Chr(10)
    c = c & "    On Error Resume Next" & Chr(10)
...
Sub Setup_Artikel_Toolbar()
    ...
    ' (Namen sammeln, dann loeschen - sicher bei Iteration)
    Dim shp As Shape
    Dim shpDel() As String
    Dim shpN As Integer: shpN = 0
    For Each shp In wsA.Shapes
        On Error Resume Next
        If shp.TopLeftCell.Row <= 3 Then
            ReDim Preserve shpDel(shpN)
            shpDel(shpN) = shp.Name
            shpN = shpN + 1
        End If
        On Error GoTo 0
    Next shp
    ...
    wsA.Cells(2, 2).Font.Strikethrough = False
    wsA.Cells(2, 2).Font.Underline = xlUnderlineStyleNone
    wsA.Cells(2, 2).Font.Italic = False
    wsA.Cells(2, 2).Font.Bold = False
    wsA.Cells(2, 2).Borders.LineStyle = xlNone
    ...
    btns = Array( _
        ...
        Array("NEUER ARTIKEL", stahlBlau, "NeuArtikelModul.NeuerArtikel_Oeffnen"), _
    ...
    ev = ev & "    LagerMakros.Artikel_Anzahl_Anzeigen" & Chr(10)
    ...
    ev = ev & "    If Target.Row >= 5 And Target.Column >= 2 Then LagerMakros.Artikel_Zeile_Markieren Target" & Chr(10)
...
Sub GitHub_Export()
    ...
    ' Die Mappe liegt DIREKT im Git-Repo (07_Lagerverwaltung-Excel mit .git-Ordner)
    Dim EXPORT_DIR  As String: EXPORT_DIR = ThisWorkbook.Path & "\"
    Dim GIT_DIR     As String: GIT_DIR = ThisWorkbook.Path & "\"

    ' SICHERHEITSCHECK: Ohne .git-Ordner KEIN git add -A ausfuehren!
    ' (Sonst Gefahr, dass bei falschem Pfad fremde Ordner committet werden)
    ' FSO statt Dir() - Dir(vbDirectory) uebersieht versteckte Ordner wie .git
    Dim fso As Object: Set fso = CreateObject("Scripting.FileSystemObject")
    If Not fso.FolderExists(GIT_DIR & ".git") Then
        MsgBox "Kein Git-Repo gefunden in:" & vbNewLine & GIT_DIR & vbNewLine & vbNewLine & _
               "Der versteckte Ordner .git fehlt." & vbNewLine & _
               "Siehe CLAUDE.md / Pruefbericht (Befund K1).", _
               vbCritical, "GitHub Export - Abbruch"
        Exit Sub
    End If

    ' VBA-Module exportieren (inkl. CSV_Import, damit der Ordner-Stand aktuell bleibt)
    Dim moduleNames As Variant
    moduleNames = Array("LagerMakros", "NeueModule", "NeuArtikelModul", "ArtikelFix", "DuplikatPruefer", "CSV_Import")
    ...
    ' Lagerdaten als JSON exportieren (direkt aus dem Artikel-Sheet)
    Call ExportLagerJSON(EXPORT_DIR)

    ' ── Code-Schloss bauen (13.08.2026) ─────────────────────────────────────
    ' lager.json enthaelt VK UND EK zu rund 7.000 Artikeln. Bis heute ging diese
    ' Datei OFFEN nach GitHub - jeder konnte die Marge des ganzen Sortiments
    ' ablesen. Sie ist jetzt in der .gitignore gesperrt und bleibt hier liegen;
    ' die Daten wandern VERSCHLUESSELT in die index.html.
    ' Schlaegt der Bau fehl, wird NICHT hochgeladen - sonst laege oben eine Seite,
    ' die zum alten Datenstand gehoert, oder schlimmer: der Klartext.
    Dim sh As Object
    Set sh = CreateObject("WScript.Shell")

    Dim bauCmd As String, rcBau As Long
    bauCmd = "cmd /c cd /d """ & GIT_DIR & """ && node _lager-quelle\lager_bauen.js"
    rcBau = sh.Run(bauCmd, 1, True)
    If rcBau <> 0 Then
        MsgBox "Das Code-Schloss konnte nicht gebaut werden (Fehler-Code " & rcBau & ")." & vbNewLine & vbNewLine & _
               "Es wurde NICHTS hochgeladen." & vbNewLine & _
               "Haeufigste Ursache: Node.js fehlt, oder lager.json ist leer.", _
               vbCritical, "GitHub Export - Abbruch"
        Exit Sub
    End If
...
Private Sub ExportLagerJSON(targetDir As String)
    ' Liest DIREKT aus dem Artikel-Sheet (layoutunabhaengig per Spalte_Finden).
    ' V3-Layout: Zeile 4 = Ueberschriften, Daten ab Zeile 5.
    ' Werte als Strings (so erwartet es die index.html-Webansicht, Komma-Dezimal ok).
    Dim wsA As Worksheet: Set wsA = LagerMakros.GetSheet("Artikel")
    If wsA Is Nothing Then
        MsgBox "Artikel-Sheet nicht gefunden - lager.json NICHT exportiert!", vbExclamation
        Exit Sub
    End If
    ...
    If cArt = 0 Then
        MsgBox "Spalte ARTIKEL nicht gefunden - lager.json NICHT exportiert!", vbExclamation
        Exit Sub
    End If
    ...
    json = "[" & vbLf
    Dim i As Long, lfdNr As Long: lfdNr = 0
    For i = 5 To lastRow                       ' Daten ab Zeile 5 (Zeile 4 = Header!)
        If Trim(CStr(wsA.Cells(i, cArt).Value)) <> "" Then
            lfdNr = lfdNr + 1
            If Not isFirst Then json = json & "," & vbLf
            isFirst = False
            json = json & "  {" & _
                """nr"":""" & lfdNr & """," & _
                """artnr"":" & JZelle(wsA, i, cNr) & "," & _
                """artikel"":" & JZelle(wsA, i, cArt) & "," & _
                """ean"":" & JEan(wsA, i, cEAN) & "," & _
                """vk"":" & JZelle(wsA, i, cVK) & "," & _
                """ek"":" & JZelle(wsA, i, cEK) & "," & _
                """bestand"":" & JZelle(wsA, i, cAnz) & "," & _
                """einheit"":" & JZelle(wsA, i, cEinh) & "," & _
                """lagerort"":" & JZelle(wsA, i, cLag) & "," & _
                """warengruppe"":" & JZelle(wsA, i, cWG) & "," & _
                """attribut"":" & JZelle(wsA, i, cAttr) & "}"
        End If
    Next i
    json = json & vbLf & "]"

    ' UTF-8 schreiben (sonst wird das Euro-Zeichen im Browser zu Muell)
    SchreibeUTF8 targetDir & "lager.json", json
End Sub

' Zelle als JSON-String; Spalte 0 (nicht gefunden) -> leerer String
Private Function JZelle(ws As Worksheet, zeile As Long, spalte As Long) As String
    If spalte = 0 Then JZelle = """""": Exit Function
    JZelle = JStr(ws.Cells(zeile, spalte))
End Function

' EAN immer als 13-stelliger Text (verhindert 2,00000000000001E+12)
Private Function JEan(ws As Worksheet, zeile As Long, spalte As Long) As String
    If spalte = 0 Then JEan = """""": Exit Function
    Dim r As Variant: r = ws.Cells(zeile, spalte).Value
    If IsNumeric(r) And Trim(CStr(r)) <> "" Then
        JEan = """" & Format(CDbl(r), "0000000000000") & """"
    Else
        JEan = JStr(ws.Cells(zeile, spalte))
    End If
End Function

' Text als UTF-8 ohne BOM speichern (ADODB.Stream)
Private Sub SchreibeUTF8(pfad As String, inhalt As String)
    ...
    st.Position = 0
    st.Type = 1                ' Binaer
    st.Position = 3            ' BOM (3 Bytes) ueberspringen
    Dim bin As Object: Set bin = CreateObject("ADODB.Stream")
    bin.Type = 1
    bin.Open
    st.CopyTo bin
    bin.SaveToFile pfad, 2     ' 2 = ueberschreiben
    bin.Close
    st.Close
End Sub
