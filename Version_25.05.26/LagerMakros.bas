...
Sub Schnellansicht_Handler(ByVal Target As Range)
    ...
    If col < 4 Or col > 8 Then Exit Sub
    ...
    Select Case col
        ...
        Case 8: NeueModule.Schnellansicht_Schliessen
    End Select
Fehler:
    Application.EnableEvents = True
    Application.ScreenUpdating = True
...
Sub Schnellansicht_Aktualisieren()
    ...
    ' Zuerst alle Zeilen einblenden, dann loeschen
    ...
    ' Spalten: A=EAN, B=Artikel, C=VK-Preis, D=Menge, E=EK-Preis, F=Zeilenverweis
    If colArt = 0 Then
        Application.ScreenUpdating = True
    ...
    For i = 3 To lastRow
        If wsA.Cells(i, colArt).Value <> "" Then
            ...
            If colAnz > 0 Then wsS.Cells(sRow, 4).Value = wsA.Cells(i, colAnz).Value Else wsS.Cells(sRow, 4).Value = 0
            If colEK > 0 Then wsS.Cells(sRow, 5).Value = wsA.Cells(i, colEK).Value Else wsS.Cells(sRow, 5).Value = 0
            wsS.Cells(sRow, 6).Value = i  ' Zeilenverweis fuer Doppelklick
            sRow = sRow + 1
        End If
    Next i

    Application.ScreenUpdating = True
    Application.StatusBar = "Schnellansicht: " & (sRow - 4) & " Artikel aktualisiert."
End Sub
...
' ================================================================
'  ARTIKEL-SHEET: Suchen, Markieren, Aktualisieren
' ================================================================
Sub Artikel_Suchen()
    Dim wsA As Worksheet: Set wsA = GetSheet("Artikel")
    If wsA Is Nothing Then Exit Sub
    ...
    suchbegriff = Trim(CStr(wsA.Cells(2, 2).Value))
    ...
    Dim oShape As Shape
    On Error Resume Next
    Set oShape = wsA.Shapes("trefferAnzeige")
    On Error GoTo 0
    ...
    If suchbegriff = "" Then
        ...
        If Not oShape Is Nothing Then oShape.TextFrame.Characters.Text = "Alle"
        Application.ScreenUpdating = True
        Exit Sub
    End If

    Dim woerter() As String
    woerter = Split(LCase(suchbegriff), " ")
    Dim treffer As Long: treffer = 0
    Dim i As Long

    For i = 5 To lastRow
        Dim artName As String: artName = LCase(CStr(wsA.Cells(i, colArt).Value))
        Dim eanStr  As String
        If colEAN > 0 Then eanStr = CStr(wsA.Cells(i, colEAN).Value) Else eanStr = ""

        Dim isMatch As Boolean: isMatch = True
        Dim w As Integer
        For w = 0 To UBound(woerter)
            If woerter(w) <> "" Then
                If InStr(artName, woerter(w)) = 0 And InStr(eanStr, woerter(w)) = 0 Then
                    isMatch = False
                    Exit For
                End If
            End If
    ...
    If Not oShape Is Nothing Then oShape.TextFrame.Characters.Text = treffer & " Treffer"
    Application.ScreenUpdating = True
End Sub
...
Sub Artikel_Suche_Leeren()
    ...
    wsA.Rows("5:" & lastRow).Hidden = False
    On Error Resume Next
    wsA.Shapes("trefferAnzeige").TextFrame.Characters.Text = "Alle"
    On Error GoTo 0
    Application.ScreenUpdating = True
End Sub
...
Sub Artikel_Aktualisieren()
    ...
    Application.StatusBar = "Aktualisiert."
End Sub

Sub Artikel_Zeile_Markieren(Target As Range)
    Dim wsA As Worksheet: Set wsA = GetSheet("Artikel")
    If wsA Is Nothing Then Exit Sub
    Dim colArt As Long: colArt = Spalte_Finden(wsA, "ARTIKEL")
    If colArt = 0 Then Exit Sub
    ...
    Dim i As Long
    For i = 5 To lastRow
        If Not wsA.Rows(i).Hidden Then
            If i Mod 2 = 0 Then
                wsA.Rows(i).Interior.Color = RGB(242, 242, 242)
            Else
    ...
    If Target.Row >= 5 Then
        wsA.Rows(Target.Row).Interior.Color = RGB(197, 217, 241)
    End If
    Application.ScreenUpdating = True
End Sub
