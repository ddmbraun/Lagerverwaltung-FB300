...
' ------------------------------------------------------------
' Exportiert Artikel-Liste fuer PAM Mobil als artikel.json
' Quelle: Artikel-Sheet (vollstaendige Daten: EAN, Einheit, WG, VK)
' Format: [["Artikelname","Einheit","Warengruppe",vk,"EAN"], ...]
...
Private Sub ExportArtikelPAMJson(sheetName As String)
    ...
    ' Immer aus Artikel-Sheet exportieren (hat EAN, Einheit, Warengruppe, VK-PREIS)
    Dim ws As Worksheet
    ...
    ' Spalten per Name suchen - robust gegen Umstrukturierungen
    Dim colArt  As Long: colArt = LagerMakros.Spalte_Finden(ws, "ARTIKEL")
    Dim colVK   As Long: colVK = LagerMakros.Spalte_Finden(ws, "VK-PREIS €")
    Dim colEAN  As Long: colEAN = LagerMakros.Spalte_Finden(ws, "EAN13")
    Dim colEinh As Long: colEinh = LagerMakros.Spalte_Finden(ws, "EINHEIT")
    Dim colWG   As Long: colWG = LagerMakros.Spalte_Finden(ws, "WARENGRUPPE")

    If colArt = 0 Then
        MsgBox "Spalte ARTIKEL nicht gefunden! Bitte Artikel-Sheet pruefen.", vbCritical
        Exit Sub
    End If

    Dim lastRow As Long
    lastRow = ws.Cells(ws.Rows.Count, colArt).End(xlUp).Row
    ...
    ' Alle Variablen ausserhalb der Schleife deklarieren (VBA-Pflicht)
    Dim i       As Long
    ...
    Dim rawEAN  As Variant

    For i = 5 To lastRow   ' Zeile 4 = Kopfzeile, Daten ab Zeile 5

        artName = Trim(CStr(ws.Cells(i, colArt).Value))
        If artName = "" Then GoTo NextRowPAM

        ' Einheit und Warengruppe
        If colEinh > 0 Then einheit = Trim(CStr(ws.Cells(i, colEinh).Value)) Else einheit = ""
        If colWG > 0 Then gruppe = Trim(CStr(ws.Cells(i, colWG).Value)) Else gruppe = ""

        ' VK-Preis: als Zahl exportieren (Punkt als Dezimaltrennzeichen fuer JSON)
        If colVK > 0 Then vkCell = ws.Cells(i, colVK).Value Else vkCell = ""
        ...
        ' EAN als 13-stellige Zeichenkette (fuehrende Nullen erhalten)
        eanVal = ""
        If colEAN > 0 Then
            rawEAN = ws.Cells(i, colEAN).Value
            If IsNumeric(rawEAN) And CDbl(rawEAN) > 0 Then
                eanVal = Format(CDbl(rawEAN), "0000000000000")
            Else
                eanVal = Trim(CStr(rawEAN))
            End If
        End If
